...
class LogicaNegocio:
    ...
    def sp_insertar_venta(self, fecha):
        ...
        try:
            ...
            con.commit()
            id_venta = resultado[1]  # El segundo parámetro es el ID de la venta
            return id_venta
        except Error as e:
            raise e

    def sp_insertar_detalle_venta(self, id_venta, id_producto, cantidad, precio_unitario):
        """Inserta un detalle de venta llamando al procedimiento sp_insertar_detalle_venta."""
        con = self.bd.obtener_conexion()
        cursor = con.cursor()
        try:
            cursor.callproc('sp_insertar_detalle_venta', [id_venta, id_producto, cantidad, precio_unitario])
            con.commit()
        except Error as e:
            raise e

    # MÉTODOS PARA REPORTES (consultas directas)
    ...
    def obtener_reporte_ventas_mes_anio(self, mes, anio):
        ...
        Se devuelve el nombre del producto y la suma de las cantidades vendidas.
        """
        consulta = """
